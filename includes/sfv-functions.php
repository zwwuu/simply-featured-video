...
function sfv_get_the_post_video( $post = null, $attr = '' ) {
					...
					if ( sfv_fs()->can_use_premium_code() ) {
						if ( $video['useFeaturedImageAsPoster'] ) {
							...
							if ( ! empty( $video_settings['poster'] ) && is_string( $video_settings['poster'] ) ) {
								$attr['poster'] = $video_settings['poster'];
							}
							if ( ! empty( $video_settings['poster'] ) && is_numeric( $video_settings['poster'] ) ) {
								$poster_url = wp_get_attachment_url( $video_settings['poster'] );
								if ( $poster_url ) {
									$attr['poster'] = $poster_url;
								}
							}
						}
					}
	 ...
	 *
	 */
	return apply_filters( 'sfv_post_video_html', $html, $post->ID, $attr );
}
