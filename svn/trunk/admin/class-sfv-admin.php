...
class SFV_Admin {
	...
	public function enqueue_block_assets() {
		...
		if ( isset( $options['post_types'] ) && ! empty( $options['post_types'] ) ) {
			...
			if ( in_array( $current_post_type, $post_types ) ) {
				wp_enqueue_style( $this->plugin_name, plugins_url( 'build/index.css', __DIR__ ), array(), $this->version, 'all' );

				$asset_file = include( plugin_dir_path( __DIR__ ) . 'build/index.asset.php' );

				wp_enqueue_script( $this->plugin_name, plugin_dir_url( __DIR__ ) . 'build/index.js', $asset_file['dependencies'], $asset_file['version'], false );
	...
	public function add_plugin_options() {
		...
		add_settings_section( $this->plugin_name . '_general_section', __( 'General', 'simply-featured-video' ),
			array(
				$this,
		...
		add_settings_field( $this->plugin_name . '_post_types', __( 'Select the post types', 'simply-featured-video' ),
			array(
				$this,
				'display_post_types_field'
			),
			$this->plugin_name,
	...
	function display_post_types_field() {
        ...
        <label>
            <input type="checkbox"
                   id="<?php esc_attr_e( $this->plugin_name . '_options[post_types][' . $post_obj->name . ']' ); ?>"
                   name="<?php esc_attr_e( $this->plugin_name . '_options[post_types][]' ); ?>"
		...
		if ( sfv_fs()->is__premium_only() ) {
			if ( sfv_fs()->can_use_premium_code() ) {
				...
				$html = '';
				foreach ( $post_types as $post_type ) {
					$html .= sprintf( '<label><input type="checkbox" id="%s" name="%s" value="%s" %s />%s</label><br/>',
						esc_attr( $this->plugin_name . '_options[post_types][' . $post_type->name . ']' ),
						esc_attr( $this->plugin_name . '_options[post_types][]' ),
						esc_attr( $post_type->name ),
						in_array( $post_type->name, $value ) ? 'checked' : '',
